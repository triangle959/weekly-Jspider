...
var localStorageUtil = {
    save: function(name, value, k1, k2) {
        var text = JSON.stringify(value);
        text = BASE64.encrypt(text);
        text = AES.encrypt(text, k1, k2);
        try {
    ...
    getValue: function(name, k1, k2) {
        var text = localStorage.getItem(name);
        var result = null;
        if (text) {
            text = AES.decrypt(text, k1, k2);
...
function getDataFromLocalStorage(key, period, k1, k2) {
    if (typeof period === 'undefined') {
        period = 0
    }
    var d = DES.encrypt(key);
    d = BASE64.encrypt(key);
    var data = localStorageUtil.getValue(key, k1, k2);
    if (data) {
...
function get_parma(method, obj, k1, k2, k3) {
    var appId = k1;
    var clienttype = 'WEB';
    ...
    param = AES.encrypt(param, k2, k3);
    return param
}
//解密data
function decrypt_data(data, k1, k2, k3, k4) {
    data = AES.decrypt(data, k1, k2);
    data = DES.decrypt(data, k3, k4);
